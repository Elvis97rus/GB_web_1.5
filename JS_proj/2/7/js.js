...
const snake = {
  ...
  getNextStepHeadPoint() {
    ...
    // Если нынешняя точка - край карты, переходим на противоположный край
    switch (this.direction) {
      case 'up':
          return (firstPoint.y === 0)?
              {x: firstPoint.x, y: this.settings.rowsCount-1}:{x: firstPoint.x, y: firstPoint.y - 1};
      case 'right':
        return (firstPoint.x === this.settings.colsCount-1)?
            {x: 0,y: firstPoint.y}:{x: firstPoint.x + 1, y: firstPoint.y};
      case 'down':
        return (firstPoint.y === this.settings.rowsCount-1)?
            {x: firstPoint.x, y: 0}:{x: firstPoint.x, y: firstPoint.y + 1};
      case 'left':
        return (firstPoint.x === 0)?
            {x: this.settings.colsCount-1, y: firstPoint.y}:{x: firstPoint.x - 1, y: firstPoint.y};
    }
  },
...
const game = {
   ...
   */
  getStartSnakeBody() {
    let thirdOfField = this.config.getColsCount() / 3;
    return [{
      x: Math.floor(Math.random() * thirdOfField + thirdOfField),
      y: Math.floor(Math.random() * thirdOfField + thirdOfField)
    }];
  },
  ...
  //увеличение счета с увеличением длинны змейки
  scoreInc(){
  ...
  //отображение счета
  scoreRender(){
    this.config.scoreEl.innerText = this.score;
  },
